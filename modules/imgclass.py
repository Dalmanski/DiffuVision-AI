...
start = 0
real_text_features = text_features[start:start + REAL_COUNT]
start += REAL_COUNT
anime_text_features = text_features[start:start + ANIME_COUNT]
start += ANIME_COUNT
three_d_text_features = text_features[start:start + THREE_D_COUNT]
start += THREE_D_COUNT
cartoon_text_features = text_features[start:start + CARTOON_COUNT]
...
def classify_image(file_path):
    ...
    with torch.no_grad():
        ...
        real_score = get_class_score(image_features, real_text_features)
        anime_score = get_class_score(image_features, anime_text_features)
        three_d_score = get_class_score(image_features, three_d_text_features)
        cartoon_score = get_class_score(image_features, cartoon_text_features)
        ...
        results = {
            ...
            ANIME: probabilities[1].item(),
            THREE_D: probabilities[2].item(),
            CARTOON: probabilities[3].item()
        }
